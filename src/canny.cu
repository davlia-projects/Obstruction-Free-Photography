...
__global__ void kernGradient(int N, int width, int height, unsigned char * in, unsigned char * gradient, unsigned char * edgeDir) {
  int x = (blockIdx.x * blockDim.x) + threadIdx.x;
	int y = (blockIdx.y * blockDim.y) + threadIdx.y;
	if (x >= width || y >= height) {
		return;
...
unsigned char * Canny::edge(int N, int width, int height, unsigned char * in) {
  ...
  kernSmooth<<<blocksPerGrid2d, blockSize2d>>>(N, width, height, in, smooth, gaussian, 5);
  kernGradient<<<blocksPerGrid2d, blockSize2d>>>(N, width, height, smooth, gradient, edgeDir);
