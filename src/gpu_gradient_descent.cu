...
__device__
int idx(int x, int y, int width, int height) {
...
int warpIdx(int width, int height, int x, int y, int z, glm::vec2 warp) {
  int nx = clamp(x + warp.x, 0, width - 1);
  int ny = clamp(y + warp.y, 0, height - 1);
  return z * width * height + ny * width + nx;
}

__device__
float grad(float * field, int x, int y, int width, int height) {
...
__global__
void kernImageGradientUpdate(float learningRate, int width, int height, ImgData imgData, GradData gradData) {
  ...
  int idx = y * width + x;
  imgData.imgO[idx] -= learningRate * gradData.imgO_grad[idx];
  imgData.imgB[idx] -= learningRate * gradData.imgB_grad[idx];
  imgData.alpha[idx] -= learningRate * gradData.alpha_grad[idx];
  return;
}
...
void kernComputeImageGradients1(int width, int height, int frames, ImgData imgData, GradData gradData, ImgWs imgWs) {
  ...
  gradData.imgO_grad[warpO_idx] -= dataTerm * imgWs.W1[idx2];
  gradData.alpha_grad[warpO_idx] -= dataTerm * imgData.imgB[warpB_idx] * imgWs.W1[idx2];
  gradData.imgB_grad[warpB_idx] -= dataTerm * imgData.alpha[warpO_idx] * imgWs.W1[idx2];
  return;
}
__global__
void kernComputeImageGradients2(int width, int height, ImgData imgData, GradData gradData, ImgWs imgWs) {
  ...
  int idx = y * width + x;
  gradData.alpha_grad[idx] += 
    grad(imgData.alpha, x + 1, y, width, height).x + 
    grad(imgData.alpha, x - 1, y, width, height).x + 
    grad(imgData.alpha, x, y + 1, width, height).y + 
    grad(imgData.alpha, x, y - 1, width, height).y;

  glm::vec2 g;
  g = grad(imgData.imgO, x + 1, y);
  gradData.imgO_grad[idx] -= LAMBDA_2 * imgWs.W2[idx(x + 1, y, width, height)] * g.x;
  gradData.imgO_grad[idx] += LAMBDA_3 * (g.x * g.x + g.y * g.y) * g.x;
  g = grad(imgData.imgO, x - 1, y);
  gradData.imgO_grad[idx] += LAMBDA_2 * imgWs.W2[idx(x - 1, y, width, height)] * g.x;
  gradData.imgO_grad[idx] += LAMBDA_3 * (g.x * g.x + g.y * g.y) * g.x;
  g = grad(imgData.imgO, x, y + 1);
  gradData.imgO_grad[idx] -= LAMBDA_2 * imgWs.W2[idx(x, y + 1, width, height)] * g.y;
  gradData.imgO_grad[idx] += LAMBDA_3 * (g.x * g.x + g.y * g.y) * g.y;
  g = grad(imgData.imgO, x, y - 1);
  gradData.imgO_grad[idx] += LAMBDA_2 * imgWs.W2[idx(x, y - 1, width, height)] * g.y;
  gradData.imgO_grad[idx] += LAMBDA_3 * (g.x * g.x + g.y * g.y) * g.y;

  g = grad(imgData.imgB, x + 1, y);
  gradData.imgB_grad[idx] -= LAMBDA_2 * imgWs.W3[idx(x + 1, y)] * g.x;
  g = grad(imgData.imgB, x - 1, y);
  gradData.imgB_grad[idx] += LAMBDA_2 * imgWs.W3[idx(x - 1, y)] * g.x;
  g = grad(imgData.imgB, x, y + 1);
  gradData.imgB_grad[idx] -= LAMBDA_2 * imgWs.W3[idx(x, y + 1)] * g.y;
  g = grad(imgData.imgB, x, y - 1);
  gradData.imgB_grad[idx] += LAMBDA_2 * imgWs.W3[idx(x, y - 1)] * g.y;
  ...
  if (imgData.imgO[idx] > 1.0f) {
    imgData.imgO_grad[idx] -= LAMBDA_P * (1.0f - imgData.imgO[idx]);
  }
  if (imgData.imgB[idx] > 1.0f) {
    imgData.imgB_grad[idx] -= LAMBDA_P * (1.0f - imgData.imgB[idx]);
  }
  if (imgData.alpha[idx] > 1.0f) {
    imgData.alpha_grad[idx] -= LAMBDA_P * (1.0f - imgData.alpha[idx]);
  }
  return;
}
...
void kernObjectiveOtherTerms( int width, int height, ImgData imgData, float * objective) {
  ...
  float ret = 0.0f;
  glm::vec2 alpha_grad = grad(imgData.alpha, x, y);
  ret += LAMBDA_1 * (alpha_grad.x * alpha_grad.x + alpha_grad.y * alpha_grad.y);
  glm::vec2 imgO_grad = grad(imgData.imgO, x, y);
  ret += LAMBDA_2 * (glm::abs(imgO_grad.x) + glm::abs(imgO_grad.y));
  glm::vec2 imgB_grad = grad(imgData.imgB, x, y);
...
void GradientDescent::optimizeImageComponents() {
  ...
  kernImageGradientUpdate<<<blocksPerGrid2d, blockSize2d>>>(
      this->LEARNING_RATE1, this->width, this->height, *this->imgData, *this->gradData);
}
